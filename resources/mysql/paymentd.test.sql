...
-- Wed 29 Oct 2014 04:39:30 PM CET
...
-- -----------------------------------------------------
-- Table `fritzpay_principal_test`.`project_config`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `fritzpay_principal_test`.`project_config` ;

CREATE TABLE IF NOT EXISTS `fritzpay_principal_test`.`project_config` (
  `project_id` INT UNSIGNED NOT NULL,
  `timestamp` DATETIME NOT NULL,
  `web_url` TEXT NULL,
  `callback_url` TEXT NULL,
  ...
  CONSTRAINT `fk_project_config_project_key`
    ...
    REFERENCES `fritzpay_principal_test`.`project_key` (`key`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
...
INSERT INTO `fritzpay_payment_test`.`currency` (`code_iso_4217`) VALUES ('EUR');
