...
func WithAuthCookie(h *Handler, req *http.Request, token string, f func()) func() {
	return func() {
		wr := testutil.NewResponseWriter()
		h.authenticatePaymentToken(wr, req, token)
						...
						Convey("Given a web handler", WithWebHandler(ctx, func(h *Handler) {
							Convey("Given an initialized payment", WithPayment(db, prDB, s, func(p *payment.Payment, token *payment.PaymentToken) {
								...
								Convey("Given a request", func() {
									...
									Convey("When the payment is already configured", func() {
										...
										Convey("Given not all configurations are set", func() {
											...
											Convey("Given the missing configuration can be determined", func() {
												...
												Convey("Given a valid authorization cookie", WithAuthCookie(h, req, token.Token, func() {
									...
									Convey("When the payment has metadata", func() {
										tx, err := db.Begin()
										So(err, ShouldBeNil)
										p.Metadata = map[string]string{"testmeta": "hasmeta"}
										err = s.SetPaymentMetadata(tx, p)
										So(err, ShouldBeNil)
										err = tx.Commit()
										So(err, ShouldBeNil)

										Convey("Given the handler will update the metadata", func() {
											req.Header.Set("Accept-Language", "de-DE,de;q=0.8,en-US;q=0.6,en;q=0.4")
												...
												Convey("When opening the payment", func() {
													...
													Convey("When retrieving the payment", func() {
														...
														Convey("The metadata should be preserved", func() {
															...
															So(pRet.Metadata["testmeta"], ShouldEqual, "hasmeta")
															So(pRet.Metadata[payment.MetadataKeyAcceptLanguage], ShouldNotBeEmpty)
														})
													})
												})
											}))
